InsightForge supports three separate workflows: `idea2video`, `script2video`, and `novel2video`.
...
`.working_dir/<session_id-or-run_id>/` is the artifact authority. `.insightforge/sessions.json` is only a session index. `.insightforge/memory.md` stores user preferences only.
